...
async function sendMessage() {
    ...
    try {
        ...
        const data = engine
            ? await engine.getResponse({ message: text, cid: activeChat.id })
        ...
        activeChat.messages.push({ role: "bot", text: "حصل خطأ أثناء تشغيل الشات.", image: null, ts: nowMs() });
        activeChat.updatedAt = nowMs();
        saveState(state);
        renderAll(state);
    } finally {
...
function startNewChat() {
    ...
    if (window.ESChatEngine && window.ESChatEngine.resetCtx) {
        window.ESChatEngine.resetCtx(chat.id);
    }
